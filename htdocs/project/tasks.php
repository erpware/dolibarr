...
if ($action == 'createtask' && $user->rights->projet->creer)
{
	...
	if (!$cancel)
	{
		...
		if (!$error)
		{
			...
			{
				header("Location: ".DOL_URL_ROOT.'/project/tasks/list.php'.(empty($mode) ? '' : '?mode='.$mode));
				exit;
			}
			$id = $projectid;
		}
...
elseif ($id > 0 || !empty($ref))
{
    ...
    $linktocreatetask = dolGetButtonTitle($langs->trans('AddTask'), '', 'fa fa-plus-circle', DOL_URL_ROOT.'/project/tasks.php?id='.$object->id.'&action=create'.$param.'&backtopage='.urlencode($_SERVER['PHP_SELF'].'?id='.$object->id), '', $linktocreatetaskUserRight, $linktocreatetaskParam);
	...
	$linktotasks = dolGetButtonTitle($langs->trans('GoToGanttView'), '', 'fa fa-stream paddingleft imgforviewmode', DOL_URL_ROOT.'/project/ganttview.php?id='.$object->id.'&withproject=1', '', 1, array('morecss'=>'reposition'));
