...
/**
 *	\file       htdocs/project/ganttview.php
 *	\ingroup    project
...
if (! empty($conf->use_javascript_ajax))
{
	...
	'/project/jsgantt_language.js.php?lang='.$langs->defaultlang
	);
}
...
$linktocreatetask = dolGetButtonTitle($langs->trans('AddTask'), '', 'fa fa-plus-circle paddingleft', DOL_URL_ROOT.'/project/tasks.php?id='.$object->id.'&action=create'.$param.'&backtopage='.urlencode($_SERVER['PHP_SELF'].'?id='.$object->id), '', $linktocreatetaskUserRight, $linktocreatetaskParam);

$linktolist = dolGetButtonTitle($langs->trans('GoToListOfTasks'), '', 'fa fa-list-alt paddingleft imgforviewmode', DOL_URL_ROOT.'/project/tasks.php?id='.$object->id, '', 1, array('morecss'=>'reposition'));
...
if (count($tasksarray)>0)
{
	...
	foreach($tasksarray as $key => $val)	// Task array are sorted by "project, position, dateo"
	{
		...
		//if ($s) $tasks[$taskcursor]['task_resources']='<a href="'.DOL_URL_ROOT.'/project/tasks/contact.php?id='.$val->id.'&withproject=1" title="'.dol_escape_htmltag($s).'">'.$langs->trans("List").'</a>';
		/* For JSGanttImproved */
 	...
 	if (! empty($conf->use_javascript_ajax))
	{
		...
		include_once DOL_DOCUMENT_ROOT.'/project/ganttchart.inc.php';
		print '</div>'."\n";
