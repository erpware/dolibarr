...
/**
 *	\file       htdocs/project/tasks/time.php
 *	\ingroup    project
...
if (! empty($project_ref) && ! empty($withproject))
{
	if ($projectstatic->fetch(0, $project_ref) > 0)
	{
		...
		{
			header("Location: ".DOL_URL_ROOT.'/project/tasks.php?id='.$projectstatic->id.($withproject?'&withproject=1':'').(empty($mode)?'':'&mode='.$mode));
			exit;
		}
	}
}
...
if (($id > 0 || ! empty($ref)) || $projectidforalltimes > 0)
{
	...
	if ($projectstatic->usage_bill_time)
	{
	    ...
	    //if ($user->rights->project->creer) $arrayofmassactions['predelete']='<span class="fa fa-trash paddingrightonly"></span>'.$langs->trans("Delete");
	...
	if (empty($projectidforalltimes))
	{
		...
		$linkback=$withproject?'<a href="'.DOL_URL_ROOT.'/project/tasks.php?id='.$projectstatic->id.'">'.$langs->trans("BackToList").'</a>':'';
