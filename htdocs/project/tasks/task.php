...
/**
 *	\file       htdocs/project/tasks/task.php
 *	\ingroup    project
...
if ($action == 'confirm_delete' && $confirm == "yes" && $user->rights->project->supprimer)
{
	if ($object->fetch($id, $ref) >= 0)
	{
		...
		{
			header('Location: '.DOL_URL_ROOT.'/project/tasks.php?restore_lastsearch_values=1&id='.$projectstatic->id.($withproject?'&withproject=1':''));
			exit;
		}
		else
		{
...
if (! empty($project_ref) && ! empty($withproject))
{
	if ($projectstatic->fetch('', $project_ref) > 0)
	{
		...
		{
			header("Location: ".DOL_URL_ROOT.'/project/tasks.php?id='.$projectstatic->id.(empty($mode)?'':'&mode='.$mode));
		}
	}
}
	...
	if ($object->fetch($id, $ref) > 0)
	{
		...
		*/
		...
		else
		{
			...
			$linkback=$withproject?'<a href="'.DOL_URL_ROOT.'/project/tasks.php?id='.$projectstatic->id.'&restore_lastsearch_values=1">'.$langs->trans("BackToList").'</a>':'';
