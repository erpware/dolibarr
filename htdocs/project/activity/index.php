...
/**
 *	\file       htdocs/project/activity/index.php
 *	\ingroup    project
 *	\brief      Page activite perso du module project
 */
...
$projectsListId = $projectstatic->getProjectsAuthorizedForUser($user, 0, 1);  // Return all projects I have permission on because I want my tasks and some of my task may be on a public project that is not my project
$taskstatic=new Task($db);
$tasktmp=new Task($db);
...
if (empty($conf->global->PROJECT_HIDE_TASKS) && ! empty($conf->global->PROJECT_SHOW_TASK_LIST_ON_PROJECT_AREA))
{
	...
	dol_syslog('project:index.php: affectationpercent', LOG_DEBUG);
	$resql = $db->query($sql);
	if ( $resql )
	{
		...
		while ($i < $num && $i < $max)
		{
			...
			print '</td>';
			print '<td class="right"><a href="'.DOL_URL_ROOT.'/project/tasks/time.php?id='.$obj->taskid.'&withproject=1">';
